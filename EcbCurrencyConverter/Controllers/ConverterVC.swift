...
class ConverterVC: UIViewController {
    ...
    func initialData() {
        ...
        getApiEcbLatestDate { date in
            guard let date else {
                spinner.dismissLoader()
                return
            }
            self.currentRatesDate = date
            self.lblLastUpdate.text = "Rates based on: \(date)"
            spinner.dismissLoader()
        }
    }
    ...
    func getApiEcbConvertRates(data: ConversionData) {
        let spinner = showLoader(view: self.view)
        guard let fromCurrency = data.fromCurrency,
              let date = data.convertDate,
              let amount = data.fromAmount,
              let toCurrency = data.toCurrency else {
            spinner.dismissLoader()
            return
        }
        let callUri = createConvertRatesUri(
        ...
        ApiService.shared.fetchApiData(urlString: callUri) { (rates: RatesDetailModel?, error: ErrorModel?) in
            ...
            if let rate = rates.rates.first {
                self.txtRightInput.text = "\(rate.value)"
            }
            spinner.dismissLoader()
        }
    }
